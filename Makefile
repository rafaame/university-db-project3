...
remote-dir = /var/www/university-db-project3.server.local.rafaa.me/httpdocs/
...
build:
	...
	cd src && find . -path ./vendor -prune -o -type f -a \( -name "*.html" -o -name "*.php" -o -name "*.phtml" -o -name ".htaccess" \) -exec rsync -Rv "{}" ../$(dist-dir)/ \;
...
clean:
	rm -rf $(dist-dir)/css $(dist-dir)/js $(dist-dir)/img $(dist-dir)/fonts
